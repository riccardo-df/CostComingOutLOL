...
produce_latex_pooled <- function(pooled_result_list) {
  ...
  cat("\n      \\addlinespace[3pt]
      ...
      \\end{tabular}
      \\end{adjustbox}
      \\caption{Point estimates and $95\\%$ confidence intervals for $\\hat{\\tau}$ obtained using different donor pools. The number of donors receiving a non-zero weight is reported. Additionally, the pre-treatment root mean squared error (RMSE) is reported.}
...
produce_latex_regional <- function(regional_result_list) {
  ...
  cat("      \\multicolumn{", n_col, "}{l}{\\textbf{\\small Panel 4: \\textit{North America}}} \\\\
      ...
      RMSE & ", stringr::str_sub(paste(paste0(rmses_north_am, " &"), collapse = " "), end = -3), " \\\\ \\cmidrule{", 1, "-", n_col, "} \n\n", sep = "")
  ...
  cat("\n      \\addlinespace[3pt]
      ...
      \\end{tabular}
      \\end{adjustbox}
      \\caption{Point estimates and $95\\%$ confidence intervals for $\\hat{\\tau}$ obtained using different donor pools. The number of donors receiving a non-zero weight is reported. Additionally, the pre-treatment root mean squared error (RMSE) is reported. Results are divided according to the region.}
