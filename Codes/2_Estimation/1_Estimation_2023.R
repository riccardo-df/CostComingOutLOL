...
min_date <- as.POSIXct("2023-01-01", tryFormats = "%Y-%m-%d")
max_date <- as.POSIXct("2023-07-15", tryFormats = "%Y-%m-%d")

## Set SC estimator.
donor_pools <- c("all", "non_lgb", "jungle", "middle", "top", "support", "adc")
estimator <- "sc"
treatment_date <- as.POSIXct("2023-06-01", tryFormats = "%Y-%m-%d")
inference <- FALSE
backdate <- 10
...
for (pool in donor_pools) {
  ...
  pooled_result_list[[counter]] <- run_main_pooled(champions, outcome_colname_pool, pool, estimator, treatment_date, backdate, inference = inference, bandwidth = bandwidth_pool, covariate_colnames = covariates_pool, min_date = min_date, max_date = max_date)
  regional_result_list[[counter]] <- run_main_regional(champions, outcome_colname_regional, pool, estimator, treatment_date, bandwidth = bandwidth_regional, covariate_colnames = covariates_regional, min_date = min_date, max_date = max_date)
